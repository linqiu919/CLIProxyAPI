...
export const authFilesApi = {
  ...
  // Deno 代理配置
  async getDenoProxyConfig(id: string): Promise<{ id: string; deno_host: string; enabled: boolean }> {
    const data = await apiClient.get(`/auth-files/deno-proxy?id=${encodeURIComponent(id)}`);
    return data as { id: string; deno_host: string; enabled: boolean };
  },

  async setDenoProxyConfig(ids: string[], denoHost: string): Promise<{ updated: string[]; failed: string[] }> {
    const data = await apiClient.post('/auth-files/deno-proxy', {
      ids,
      deno_host: denoHost
    });
    return data as { updated: string[]; failed: string[] };
  },

  async deleteDenoProxyConfig(ids: string[]): Promise<{ deleted: string[]; failed: string[] }> {
    const data = await apiClient.delete('/auth-files/deno-proxy', {
      data: { ids }
    });
    return data as { deleted: string[]; failed: string[] };
  },

  // 启用/禁用认证文件
  async patchAuthFileStatus(name: string, disabled: boolean): Promise<void> {
    await apiClient.patch('/auth-files/status', { name, disabled });
  }
};
